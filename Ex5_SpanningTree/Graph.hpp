...
class CountingVisitor : Visitor<Vertex>
{
    ...
    public:
        ...
        { 
            // std::cout << "Visited [" << element.Number() << "]\n";
            ctr++;
        }
...
class GraphAsMatrix
{
    ...
    public:
        ...
        {
            std::cout << "DFS1: " << v->Number() << std::endl;
        ...
        bool IsConnected()
        {
            ...
            if(!this->isDirected) {
                ...
                return cv.counter() == this->numberOfVertices;
            } else {
                for(int i = 0; i<numberOfVertices; i++) {
                    std::vector<bool> visited(this->numberOfVertices, false);
                    DFS1(&cv, vertices[i], visited);
                    if(cv.counter() != this->numberOfVertices) return false;
                    cv.reset(); 
                }
                return true;
            }
        ...
        void DFS_Spanning_Tree_1(Vertex *v, std::vector<bool> &visited, std::vector<int> &parent)
        {
            ...
            while(!it.IsDone()) {
                if(!visited[(*it).V1()->Number()]) {
                    parent[(*it).V1()->Number()] = v->Number();
                    std::cout << "Rodzicem \033[32m" << (*it).V1()->Number() 
                            << "\033[0m jest \033[31m" << v->Number() << "\033[0m\n"; 
                    DFS_Spanning_Tree_1((*it).V1(), visited, parent);
                }
                ++it;
            }    
        }
